...
- Playwright browser scenarios для protected routes, login flow и фильтрации профиля
- GitHub Actions pipeline под lint/test/build и публикацию образов в GHCR
- Kubernetes manifests и bootstrap script для namespace `polka` на VPS с `k3s`, `traefik` и `cert-manager`
...
## Следующие шаги
1. Прогнать branch publish в GHCR и верифицировать первый rollout Polka в namespace `polka`.
2. Расширить Playwright-сценарии до owner CRUD для книг и списков.
3. Подготовить финальный handoff по продовому rollout и rollback.
4. Подготовить Lighthouse-замеры и production polish.
