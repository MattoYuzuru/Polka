...
Текущий статус: собран рабочий auth/profile/books/recommendation-lists foundation с Angular 21, Taiga UI 4, Signal Store, Go API, PostgreSQL и вертикальным срезом `register/login -> JWT session -> owner/guest profile -> create/edit/delete book -> reorder/filter library -> create/edit/delete recommendation list`.
...
## Следующие шаги
1. Добавить вычисляемую статистику чтения за период и более богатый analytics-block на профиле.
2. Настроить ESLint, Stylelint, Prettier, Jest и Playwright component tests.
3. Собрать CI/CD pipeline с публикацией образов в GHCR и deploy flow на VPS.
4. Подготовить Lighthouse-замеры и production polish перед финальным README/deploy handoff.
